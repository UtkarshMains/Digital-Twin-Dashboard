...
def mon():
    ...
    dfi.export(var_cost, "../app/src/image/vc.png", table_conversion="matplotlib")
    # Factory Fixed Costs
    ...
    d=df_demand.head(15)
    dfi.export(d, "../app/src/image/stages.png", table_conversion="matplotlib")
    ...
    demand_var.columns = ['Initial'] + list(range(1, N + 1))
    print(demand_var)

    print(demand_var.columns)
    l=[]
    # Loop
    for scenario in range(1, 51):  # 0 is the initial scenario

        # Calculations
        status_out, objective_out, y, x, fix, var = optimization_model(fixed_costs, var_cost, demand_var[[scenario]],
                                                                       scenario, cap)

        # Append results
        list_status.append(status_out)
        list_results.append(objective_out)
        df_bool[scenario] = [y[plant_name[idx]].varValue for idx in range(len(plant_name))]
        list_fixcost.append(
            sum([fixed_costs.loc[location, s] * y[(location, s)].varValue * 1000 for s in size for location in loc]))
        list_varcost.append(sum([var_cost.loc[loc1, loc2] * x[(loc1, loc2)].varValue for loc1 in loc for loc2 in loc]))
        total_demand = demand_var[scenario].sum()
        list_totald.append(total_demand)
        list_scenario.append(scenario)

        # Print the total cost for this scenario
        print(f"Status for scenario {scenario}: {status_out}, Total cost of scenario: {objective_out}, Total demand for scenario {scenario}: {total_demand}")
        l.append("Status for scenario "+str(scenario)+ " : "+str(status_out)+" ,  Total cost of scenario: "+str(objective_out)+" , Total demand for scenario"+str(scenario)+" : " +str(total_demand))
    # Boolean
    dfl = pd.DataFrame(l, columns=[''])
    dfi.export(dfl, "../app/src/image/totalcost.png", table_conversion="matplotlib")
    ...
    df_comb = df_comb.set_index([pd.Index(COL_NAME)])
    print(df_comb)
